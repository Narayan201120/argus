...
def _key(request_id: str, owner: str = "local") -> str:
    return f"argus:feedback:{owner}:{request_id}"


def _inv_key(investigation_id: str, owner: str = "local") -> str:
    return f"argus:feedback:{owner}:inv:{investigation_id}"


async def save_rating(request_id: str, rating: int, *, owner: str = "local") -> bool:
    ...
    if client is None:
        return False
    try:
        payload: dict[str, Any] = {"rating": rating, "ts": time.time(), "owner": owner}
        ttl = max(settings.feedback_ttl_s, 60)
        await client.set(_key(request_id, owner), json.dumps(payload), ex=ttl)
        return True
...
async def get_rating(request_id: str, *, owner: str = "local") -> int | None:
    if not settings.feedback_enabled or not request_id:
        return None
    client = holder.client
    if client is None:
        return None
    try:
        raw = await client.get(_key(request_id, owner))
...
async def save_investigation_rating(
    investigation_id: str, rating: int, *, owner: str = "local"
) -> bool:
    ...
    try:
        payload: dict[str, Any] = {"rating": rating, "ts": time.time(), "owner": owner}
        ttl = max(settings.feedback_ttl_s, 60)
        await client.set(_inv_key(investigation_id, owner), json.dumps(payload), ex=ttl)
        return True
    except Exception as exc:  # noqa: BLE001 - fail open
...
async def get_investigation_rating(
    investigation_id: str, *, owner: str = "local"
) -> int | None:
    if not settings.feedback_enabled or not investigation_id:
        return None
    client = holder.client
    if client is None:
        return None
    try:
        raw = await client.get(_inv_key(investigation_id, owner))
        return json.loads(raw)["rating"] if raw else None
    except Exception as exc:  # noqa: BLE001

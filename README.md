...
This is a deep-research platform at **v0.5.0**. Two paths: the bounded parallel `/v1/query` pipeline stays the fast default (direct or researcher/analyzer/verifier + synthesis, SSE streaming, working-memory sessions, voice in and out, A/B routing with feedback ratings), and `POST /v1/investigate` runs the adaptive research loop (parallel Radar/RAG/web tools -> Evidence Board -> analysis/critique/gap workers -> follow-up rounds under iteration, tool-call, wall-clock, and cost budgets -> milestone synthesis streamed live -> completed Markdown report). Shipped around it: deep-report mode (`POST /v1/report`), a four-view workspace UI (chat, investigations with live board + report streaming, Radar paper search, RAG document library), read-only sibling proxies, dead-provider demotion, restart-safe supervision, cost governance with `COST_LIMIT`, Prometheus metrics with a provisioned Grafana dashboard covering the full measurement set, and opt-in JWT auth plus OTel tracing. Deferred: per-user RAG authorization mapping (service identity is temporary), OAuth2, local model connectors (Ollama), Kubernetes/Helm, long-term memory recall.
